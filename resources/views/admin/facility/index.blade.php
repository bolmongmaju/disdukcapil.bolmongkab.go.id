            ...
            @can('facilities.create')
                <div class="card">
                    <div class="card-header">
                        <h4><i class="fas fa-image"></i> Upload Foto dan Keterangan Fasilitas</h4>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                            @forelse ($facilities as $no => $facility)
                                <tr>
                                    <th scope="row" style="text-align: center">{{ ++$no + ($facilities->currentPage()-1) * $facilities->perPage() }}</th>
                                    <td><img src="{{ Storage::url('public/facility-images/'. $facility->image) }}" style="width: 200px"></td>
                                    <td>{{ $facility->caption }}</td>
                                    <td class="text-center">
                                        @can('facilities.delete')
                                            <button onClick="Delete(this.id)" class="btn btn-sm btn-danger" id="{{ $facility->id }}">
    ...
    function Delete(id)
        {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    //ajax delete
                    jQuery.ajax({
                        url: "{{ route("admin.facility.index") }}/"+id,
                        data:     {
                            "id": id,
                            "_token": token
                        },
